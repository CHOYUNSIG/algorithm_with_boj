...
<pre class="mermaid">
    ...
    F1 -. <p style="color: #888">2<sup>2</sup></p> .-> B1
    F1 -. <p style="color: #888">2<sup>1</sup></p> .-> D2
    F1 -. <p style="color: #888">2<sup>0</sup></p> .-> E1
...
위 트리를 예시로 들어보자. 트리의 맨 아래 노드는 깊이가 5이고(루트의 깊이를 0으로 보고) $2^0$, $2^1$, $2^2$번째 조상이 존재하므로 노드에 이 정보를 저장해 둔다. 모든 노드가 이러한 정보를 저장하고 있다면 최소 공통 조상 문제를 빠르게 풀 수 있다. 크기가 $N$인 트리의 임의의 노드 $\text A$와 $\text B$의 최소 공통 조상 $\text C$를 찾아가는 상황을 생각해보자. 노드 $\text A$의 깊이가 더 크며 노드 $\text A$와 $\text C$ 경로 사이에 노드 $\text B$와 깊이가 같은 노드 $\text A'$이 있다. 도식화하면 아래 그림과 같으며 다른 노드는 생략하였다.

<pre class="mermaid">
    graph RL

    root --> C
    C -- <p style="color: #888">d<sub>BC</sub></p> --> Ap
    Ap -- <p style="color: #888">d<sub>AA'</sub></p> --> A
    C -- <p style="color: #888">d<sub>BC</sub></p> --> B
...
우선 노드 $\text A$에서부터 노드 $\text B$와 깊이가 같은 노드인 $\text A'$까지 거슬러 올라온다. 예시로 $d_{\text{AA}'} = 13$이라고 해보자. 이는 다음과 같이 표현할 수 있다.

$$
d_{\text{AA}'} = 13 = 1011_{2} = 2^3 + 2^1 + 2^0
$$
...
<a href="../../../2024/01/18/euler_tour_technique.html">오일러 경로 테크닉</a>이란 트리의 DFS 방문 순서대로 노드들을 나열하여 트리를 선형적으로 관리하는 기법이다. 어떠한 노드 $\text A$와 $\text B$ 사이의 최소 공통 조상은 DFS 방문 순서상 노드 $\text A$와 $\text B$ 사이에서 반드시 재방문하게 됨을 이용한다.
